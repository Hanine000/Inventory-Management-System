...
const DetailRow = ({ label, value }) => (
  <div className="flex items-start justify-between py-3 border-b border-slate-800/60 last:border-0">
    <span className="text-xs text-slate-500 font-medium uppercase tracking-wider w-36 flex-shrink-0 pt-0.5">{label}</span>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className={`text-xs font-medium ${isLow ? "text-amber-400" : "text-emerald-400"}`}>
          ...
          {images.map((src, i) => (
            <button
              ...
              className={[
                ...
                i === active ? "border-rose-500" : "border-slate-800 hover:border-slate-600",
              ].join(" ")}
            >
...
export default function OneProduct() {
  ...
  const handleEditSuccess = () => { setPanelOpen(false); refetch(); };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-slate-100 leading-tight flex-1">{product.name}</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <StatCard
              ...
              sub="units available"
              accent={product.stock <= 10 ? "text-amber-400" : "text-slate-100"}
            />
          </div>

          {/* Stock bar */}
          <div className="bg-slate-900 border border-slate-800 rounded-xl px-5 py-4">
            <StockBar stock={product.stock ?? 0} threshold={product.lowStockThreshold ?? 10} />
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl px-5 py-2">
            ...
            <DetailRow label="Brand"      value={product.brand?.name ?? product.brand} />
            <DetailRow label="Supplier"   value={product.supplier?.name ?? product.supplier} />
      ...
      {/* ── Edit slide-over ─────────────────────────────────────────────────── */}
      <SlideOver open={panelOpen} onClose={() => setPanelOpen(false)} title="Edit Product">
        {panelOpen && (
          <ProductForm
            product={product}
