 ...
 * ProductTable.jsx
 * ─────────────────────────────────────────────────────────────────
 * Renders the paginated product list.
 * Design language: dark slate + rose accent (matches ProductForm + Dashboard).
 *
 * Props:
 *   products    Product[]   current page rows
 *   total       number      total record count (for pagination label)
...
const StockCell = ({ stock }) => {
  ...
  if (quantity <= lowStockThreshold)      return <Badge color="amber">{quantity}</Badge>;
  ...
  if (primary?.url) {
    return (
      <img
        ...
        className="w-9 h-9 rounded-lg object-cover border border-slate-700/60 flex-shrink-0"
      />
    );
  }
  return (
              ...
              <tr>
                <td colSpan={9} className="text-center py-16">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs text-slate-600">Try adjusting your filters or add a new product</p>
                  </div>
                </td>
              </tr>
              ...
              products.map((product) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    {product.brand?.name && (
                      <p className="text-[11px] text-slate-500 mt-0.5">{product.brand.name}</p>
                    )}
                  </td>
                  ...
                  {/* Stock */}
                  <td className="px-4 py-3">
                    <StockCell stock={product.stock ?? { quantity: 0, lowStockThreshold: 10 }} />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                      ...
                      <ActionBtn title="Edit product" onClick={() => onEdit?.(product)}>
                      ...
                      {product.isActive ? (
                        <ActionBtn title="Deactivate product" danger onClick={() => onDelete?.(product)}>
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
                        ...
                        <ActionBtn title="Restore product" onClick={() => onRestore?.(product)}>
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
